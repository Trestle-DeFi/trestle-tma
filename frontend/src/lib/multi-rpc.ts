...
function buildBlockscoutRpc(chainId: number): RpcConfig | null {
  const key = import.meta.env.VITE_BLOCKSCOUT_API_KEY || import.meta.env.VITE_PUBLIC_BLOCKSCOUT_API_KEY || "";
  if (!key) return null;
  return { url: `https://api.blockscout.com/${chainId}/json-rpc`, label: "Blockscout PRO API", weight: 3, headers: { Authorization: `Bearer ${key}` } };
}

function getRpcs(chainId: number): RpcConfig[] {
  const custom = RPCS[chainId] ?? [];
  const blockscout = buildBlockscoutRpc(chainId);
  if (!blockscout) return custom;
  const deduped = custom.filter(r => r.url !== blockscout.url);
  return [...deduped, blockscout];
}

async function rpcCall(url: string, method: string, params: any[] = [], timeout = TIMEOUT_MS, headers: Record<string, string> = {}): Promise<any> {
...
class MultiRpcProvider {
  ...
  private checkHealth = async () => {
    ...
    await Promise.allSettled(
      rpcs.map(async (rpc) => {
        ...
        try {
          const block = await rpcCall(rpc.url, "eth_blockNumber", [], 3_000, rpc.headers);
  ...
  async request(method: string, params: any[] = []): Promise<any> {
    ...
    for (const rpc of byPriority) {
      try {
        return await rpcCall(rpc.url, method, params, TIMEOUT_MS, rpc.headers);
      } catch {
